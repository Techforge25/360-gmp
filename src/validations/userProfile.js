...
const createUserProfileSchema = joi.object({ 
    ...
    banner: joi.string().uri().optional().allow("", null).label("Banner"),
...
// Update basic info validator
const updateUserProfileBasicInfoValidator = joi.object({
    fullName: joi.string().pattern(namePattern).min(3).max(50).required().label("Full name").messages({
        "string.pattern.base": "Full name can only contain letters and spaces"
    }),
    bio: joi.string().trim().max(3000).required().label("Bio"),
...
// Update user profile resume validator
const updateUserProfileResumeValidator = joi.object({
    resumeUrl: joi.string().trim().uri().required().label("Resume URL")
});
...
updateUserProfileResumeValidator };